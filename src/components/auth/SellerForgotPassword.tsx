...
import { Store, Lock, Mail, Smartphone, ArrowRight, KeyRound } from 'lucide-react';

export const SellerForgotPassword: React.FC = () => {
  const { initiateSellerForgotPassword, resetSellerPassword, navigate } = useApp();
  ...
  const handleRequestOtp = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const res = initiateSellerForgotPassword(identifier);
      setIsLoading(false);
      if (res.success) {
        setStep('reset');
        setOtp('123456');
      } else {
        setErrorMessage(res.error || 'Failed to send OTP. Seller account not found.');
      }
    }, 400);
  };
  ...
  const handleResetPassword = (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    setTimeout(() => {
      const res = resetSellerPassword(identifier, otp, newPassword);
      setIsLoading(false);
      if (!res.success) {
    ...
    <div className="min-h-[85vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-6">
        <div className="text-center space-y-2">
          ...
          <div className="inline-block px-3 py-1 bg-amber-500 text-slate-950 font-extrabold rounded-full text-xs tracking-wider uppercase">
            Seller Account Recovery
          </div>
          <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
            {step === 'request' ? 'Reset Seller Password' : 'Enter OTP & Set New Password'}
          </h2>
          <p className="text-sm text-slate-600">
            {step === 'request'
              ? 'Enter your registered shop email or mobile number to receive OTP'
              : 'Enter the 6-digit OTP code to update your merchant login credentials'}
          </p>
        </div>
          ...
          {step === 'request' ? (
            <form onSubmit={handleRequestOtp} className="space-y-4">
              ...
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-600">
                💡 Demo seller: <strong>sharmakirana.delhi@gmail.com</strong>
              </div>
            ...
            <form onSubmit={handleResetPassword} className="space-y-4 animate-in fade-in">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-3.5 px-4 bg-emerald-600 hover:bg-emerald-500 active:scale-[0.99] text-white font-bold text-sm rounded-xl shadow-lg flex items-center justify-center gap-2 transition-all cursor-pointer disabled:opacity-50"
              >
                {isLoading ? (
                  ...
                  <>
                    ...
                    <span>Update Password & Go to Seller Login</span>
                  </>
                )}
              </button>
            </form>
