...
export const ProductListingView: React.FC = () => {
  ...
  const filteredProducts = products.filter(product => {
    ...
    if (selectedBrandFilter !== 'all') {
      const matchBrandSlug = product.brandSlug === selectedBrandFilter;
      const matchBrandId = product.brandId === selectedBrandFilter;
      const matchBrandName = product.brand.toLowerCase() === selectedBrandFilter.toLowerCase();
      if (!matchBrandSlug && !matchBrandId && !matchBrandName) return false;
    }
        ...
        <div className="flex flex-wrap items-center gap-1.5 bg-slate-100 p-1 rounded-2xl text-xs font-bold">
          <button
            ...
            className={`px-3 py-1.5 rounded-xl cursor-pointer ${
              filterType === 'all' && selectedBrandFilter === 'all'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4 bg-white p-5 rounded-3xl border border-slate-200 shadow-xs h-max">
          ...
          <div className="space-y-1 pt-2 border-t border-slate-100">
            <label className="text-[11px] font-bold uppercase text-slate-400 block mb-1">Harwalkart Brands</label>
            <button
            ...
            {brands.map(b => (
              <button
                key={b.id}
                onClick={() => setSelectedBrandFilter(b.slug)}
                className={`w-full text-left px-3 py-2 rounded-xl text-xs font-semibold flex items-center justify-between cursor-pointer ${
                  selectedBrandFilter === b.slug
